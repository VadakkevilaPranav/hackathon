    ...
    <div className="detail-page">
      <div className="detail-header">
        ...
        <div className="detail-meta">
          ...
          {job.interested_users?.length > 0 && <span className="meta-tag">👥 {job.interested_users.length} {t.interested}</span>}
        </div>
      </div>
      ...
      {poster && (
        <div className="detail-card">
          ...
          <div className="poster-row">
            {poster.photo ? <img src={poster.photo} alt={poster.name} className="poster-avatar" /> : <div className="poster-avatar-fallback">{poster.name?.[0]}</div>}
            <div>
              <div className="poster-name">{poster.name}</div>
              <div className="poster-rating">
                {poster.rating > 0 ? <><span className="stars">{renderStars(poster.rating)}</span> {poster.rating} ({poster.review_count} {t.reviews})</> : t.noReviews}
              </div>
              {poster.skills_offered?.length > 0 && <div style={{ marginTop: '0.4rem', fontSize: '0.82rem', color: 'var(--text-muted)' }}>{t.skills}: {poster.skills_offered.join(', ')}</div>}
            </div>
          </div>
        </div>
      )}
      {user && user.id !== job.posted_by && (
        <div className="action-area" style={{ marginBottom: '1rem' }}>
          {whatsappLink && <a href={whatsappLink} target="_blank" rel="noreferrer" className="btn-whatsapp">{t.contactWhatsapp}</a>}
          <button className="btn-interest" onClick={handleInterest} disabled={interested}>{interested ? t.interestExpressed : t.expressInterest}</button>
          <button className="filter-btn" style={{ textAlign: 'center' }} onClick={() => setShowReview(!showReview)}>{t.leaveReview}</button>
        </div>
      )}
      {showReview && (
        <div className="detail-card">
          ...
          <textarea rows={3} placeholder={t.shareExperience} value={comment} onChange={e => setComment(e.target.value)}
          ...
          <button className="btn-primary" style={{ marginTop: '0.8rem' }} onClick={handleReview} disabled={reviewSubmitting}>{reviewSubmitting ? t.submitting : t.submitReview}</button>
        </div>
      )}
